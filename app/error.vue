...
const errorHint = computed(() => {
  ...
  if (!msg || typeof msg !== 'string' || msg.length < 4 || /^\d+$/.test(msg)) {
...
<template>
  <div class="flex min-h-screen flex-col items-center justify-center bg-[#0b0f17] px-4 text-center text-white">
    <div class="mb-4 text-6xl font-bold text-brand-600">{{ statusCode }}</div>
    ...
    <button class="btn-primary-solid mt-8" @click="goHome">
      {{ t('errors.home') }}
    </button>
  </div>
</template>
