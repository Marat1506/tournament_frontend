...
<template>
  <div>
    ...
    <div class="page-container space-y-3 !pt-0">
      ...
      <div v-else-if="loadError" class="card space-y-3 p-4">
        <AppAlert type="error" :message="t(getCommonApiErrorKey(loadError) ?? 'notifications.loadFailed')" />
      ...
      <template v-else>
        ...
        <p v-if="!data?.data?.length" class="py-8 text-center text-sm text-gray-500">
